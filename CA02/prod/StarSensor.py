...
class StarSensor(object):
    ...
    def serviceRequest(self):
        ...
        rightAscentionCenterPoint = sensorPosition[0] 
        declinationCenterPoint= sensorPosition[1] 
        fieldOfView = self.fov       #print sensorPosition  
        
        if self.isSunInFieldOfView(rightAscentionCenterPoint, declinationCenterPoint, fieldOfView):
            return '8000'   # i.e., -32767
        
        mag = self.getMagnitude(rightAscentionCenterPoint, declinationCenterPoint, fieldOfView)
    ...
    def isSunInFieldOfView(self, rightAscentionCenterPoint, declinationCenterPoint, fieldOfView):
        if (rightAscentionCenterPoint - fieldOfView / 2.0 <= 0) or (rightAscentionCenterPoint + fieldOfView / 2.0 >= 2 * math.pi):
            if (declinationCenterPoint >= 0 and declinationCenterPoint - fieldOfView / 2.0 <= 0) or (declinationCenterPoint <= 0 and declinationCenterPoint + fieldOfView / 2.0 >= 0):
                return True
        return False